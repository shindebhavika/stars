...
function Card({ title, description, imageUrl, planetData }) {
  const { setknowMorePlanet ,setImgurl} = useContext(PlanetContext);
  ...
  return (
    <div
      className="card rounded-md mt-5 min-h-[33rem] cursor-pointer border border-black-800 animate__animated animate__fadeInUp transition-all duration-300 ease-in-out "
      style={{ width: "18rem" }}
      onClick={handleClick}>
      <img
        src={imageUrl}
